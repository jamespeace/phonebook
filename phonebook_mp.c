...
/* create memory pool */
void mempool(int memNumber)
{
	
	head = (entry *) malloc(sizeof(entry) * memNumber);	
	if (head == NULL) {
		perror("can't malloc edquate data\n");
		exit(0);
	}
	end = head + memNumber;
	current = head;
...
entry *mpalloc(int memSize)
{
	int size = memSize / sizeof(entry);
...
int append_hash(char *line, hashTable *ht)
{
	...
	/* initial an entry */
	if ((e = (entry *) mpalloc(sizeof(entry))) == NULL) {
